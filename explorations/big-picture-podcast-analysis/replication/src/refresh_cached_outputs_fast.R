...
transcript_rows <- vector("list", nrow(analysis_sample))
manifest_rows <- vector("list", nrow(analysis_sample))

for (i in seq_len(nrow(analysis_sample))) {
  ep <- analysis_sample[i, ]
  cache_file <- ep$cache_path[[1]]
  ...
  manifest_rows[[i]] <- tibble(
    ...
    episode_type = ep$episode_type,
    transcript_url = ep$transcript_url,
    transcript_available = nrow(usable_segments) > 0,
  ...
  transcript_rows[[i]] <- usable_segments |>
    mutate(
      slug = ep$slug,
      title = ep$title,
      episode_date = ep$episode_date,
...
transcript_manifest <- bind_rows(manifest_rows) |>
...
results_memo <- c(
  ...
  sprintf("- Cached transcript pages available: %s of %s target episodes.", sum(transcript_manifest$transcript_available, na.rm = TRUE), nrow(transcript_manifest)),
  sprintf("- Usable cached transcript segments: %s.", sum(transcript_manifest$usable_segments, na.rm = TRUE)),
...
run_summary <- tibble(
  ...
  value = c(
    ...
    sum(transcript_manifest$transcript_available, na.rm = TRUE),
    sum(transcript_manifest$usable_segments, na.rm = TRUE),
    nrow(movie_scores)
  )
)
